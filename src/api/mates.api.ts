...
export const fetchReceivedMateRequests = async (user_id: number) => {
  const response = await httpClient.get(`/api/users/${user_id}/mates/requests/received`);
  return response.data;
};

// 친구 요청을 수락하는 API 호출 
export const acceptMateRequest = async (user_id: number,requestID: number) => {
  const response = await httpClient.put(`/api/users/${user_id}/mates/requests/${requestID}`);
  return response.data;
};

// 친구 요청을 거절하는 API 호출
export const rejectMateRequest = async (user_id: number,requestID: number) => {
  const response = await httpClient.delete(`/api/users/${user_id}/mates/requests/${requestID}`);
  return response.data;
};
